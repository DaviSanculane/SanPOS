...
Public Class frmLoadCustomer
    ...
    Private Sub LoadCustomer()
        Try
            sqL = "SELECT CustomerNo, CustName, Address, ContactNo FROM Customer Order By CustName"
            ConnDB()
    ...
    Private Sub SearchCustomer()
        Try
            sqL = "SELECT CustomerNo, CustName, Address, ContactNo FROM Customer WHERE CustName LIKE '" & TextBox1.Text & "%' Order By CustName"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()

...
Public Class frmPrintReceipt
    ...
    Private Sub getCashChange()
        Try
            sqL = "SELECT Cash, Change FROM PAYMENT WHERE InvoiceNo = '" & frmPOS.lblInvoiceNo.Text & "'"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
    ...
    Private Sub Company()
        Try
            sqL = "SELECT HotelName, AddressLine1, ContactNo, Nuit, TicketFooterMessage from Company"
            ConnDB()
            cmd = New OleDbCommand(sqL, conn)
            cmd.Parameters.AddWithValue("@d1", lblCompany.Text)
            cmd.Parameters.AddWithValue("@d2", lblAddress.Text)
            cmd.Parameters.AddWithValue("@d3", lblContact.Text)
            cmd.Parameters.AddWithValue("@d4", lblNuit.Text)
            'cmd.Parameters.AddWithValue("@d5", lblThank.Text)
            dr = cmd.ExecuteReader()
            If dr.Read() Then
                lblCompany.Text = dr.GetValue(0)
                lblAddress.Text = dr.GetValue(1)
                lblContact.Text = dr.GetValue(2)
                lblNuit.Text = dr.GetValue(3)
                'lblThank.Text = dr.GetValue(4)
            End If
    ...
    Private Sub LoadReceipt()
        ...
        Try
            ...
            'Label3.Location = New Point(3, 72 + Y)
            'Label8.Location = New Point(5, 270 + Y)
            'lblThank.Location = New Point(54, 283 + Y)
            lblLine.Location = New Point(5, 299 + Y)
